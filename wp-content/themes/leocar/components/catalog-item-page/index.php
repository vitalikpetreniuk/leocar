...
<div data-product="<?=get_the_title().' '.get_field('volume',get_the_id()).', '.get_field('fuel',get_the_id())?>" class="<?=get_field('transmission',get_the_id())?get_field('transmission',get_the_id())['value']:''?> catalog-item 768:w-[calc(50%-.625rem)]" data-type="<?=get_field('type', get_the_id())['value']?>" data-model="<?=get_field('model', get_the_id())?>">
    ...
    <div class="mt-3 px-5 box-border w-[calc(100%+40px)] -ml-5 overflow-x-scroll scrollbar-hide pb-0 768:w-full 768:ml-0 768:px-0">
        <ul class="terms min-w-[100%] flex w-auto bg-car rounded-[15px] cursor-pointer">
            ...
            <?php foreach($arr as $key=>$p_price) { ?>
                <li class="<?=($key==0?'active':'')?> px-6 h-11 flex items-center whitespace-nowrap text-[12px]/[1] text-white bg-car rounded-[15px] cursor-pointer 1024:w-[<?=($key == count($arr)-1?'24':19)?>%] 1024:justify-center 1024:px-0" data-price="<?=(int)$p_price['price']?number_format($p_price['price'],0,'',' '):''?>" data-deposit="<?=$p_price['min']?>"><?=$p_price['period']?></li>
            <?php   } ?>
    ...
    <div class="flex justify-between items-baseline">
        <div class="mt-2 flex justify-start items-baseline gap-x-1">
            <span class="font-400 text-4xl text-accent"><?=(int)$arr[0]['price']?'<span class="amount">'.$arr[0]['price'].'</span> €</span>/'.esc_html__('день','leocar'):''?></div>
