...
<div data-product="<?=get_the_title().' '.get_field('volume',get_the_id()).', '.get_field('fuel',get_the_id())?>" class="<?=get_field('transmission',get_the_id())?get_field('transmission',get_the_id())['value']:''?> catalog-item 768:w-[calc(50%-.625rem)] 1024:w-[calc(33.333%-.833rem)]">
    <div class="flip-card relative flex flex-wrap items-center justify-center rounded-md aspect-[4/3] 1100:aspect-[1/.83]">
        ...
        <div class="flip-card-inner flipped rounded-2xl cursor-pointer">
            <div class="flip-card-front rounded-2xl px-4">
                ...
                <?php if(!get_field('available',get_the_id())) { ?>
                    <div class="not-available absolute z-[2]">
                        <?=get_field('na_text','options')?>
                    </div>
                <?php   } ?>
                <?php if($colors) { ?>
                <div class="colors-wrap absolute z-[2] right-5 bottom-5 justify-end items-center gap-x-2.5">
                    <div class="colors flex">
                        <?php foreach($colors as $color) { ?>
                            <a  class="w-4 h-4 border border-white rounded-full indent-[-9999px]" href="#" style="background:<?=$color['color']?>;"><?=$color['color']?></a>&nbsp;&nbsp;
                        <?php   } ?>
                    </div>
                    <div class="subtext white"><?php esc_html_e('Доступні кольори','leocar'); ?></div>
                </div>
                <?php   } ?>
            </div>
    ...
    <?php if(!get_field('available',get_the_id())) { ?>
        <button class="button mt-2 max-w-none"><?=get_field('na_button_text','options')?></button>
    <?php   }
    else { ?>
