    ...
    <div class="mt-3 px-5 box-border w-[calc(100%+40px)] -ml-5 overflow-x-scroll scrollbar-hide pb-0 768:w-full 768:ml-0 768:px-0">
        <ul class="terms min-w-[100%] flex w-auto bg-car rounded-[15px] cursor-pointer">
            ...
            <?php foreach($arr as $key=>$p_price) {  ?>
                ...
                <li class="<?=($key==0?'active':'')?> px-6 h-11 flex items-center whitespace-nowrap text-[12px]/[1] text-white bg-car rounded-[15px] cursor-pointer active cursor-pointer 1024:w-[24%] 1024:justify-center 1024:px-0" data-price="<?=(int)$p_price['price']?number_format($p_price['price'],0,'',' '):''?>"  data-deposit="<?=$p_price['min']?>"><?=$p_price['period']?></li>
                <?php } else { ?>
                <li class="<?=($key==0?'active':'')?> px-6 h-11 flex items-center whitespace-nowrap text-[12px]/[1] text-white bg-car rounded-[15px] cursor-pointer active cursor-pointer 1024:w-[19%] 1024:justify-center 1024:px-0" data-price="<?=(int)$p_price['price']?number_format($p_price['price'],0,'',' '):''?>"  data-deposit="<?=$p_price['min']?>"><?=$p_price['period']?></li>
                <?php } ?>
            <?php   } ?>
        </ul>
    </div>
